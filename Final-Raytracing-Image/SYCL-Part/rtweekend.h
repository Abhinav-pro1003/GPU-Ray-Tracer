...
inline double xorand() {
  uint64_t hi = rand();
  uint64_t lo = rand();
  uint64_t mantissa = ((hi & 0x000FFFFF) << 32) | lo;  // 52-bit mantissa

  uint64_t bits = (uint64_t(0x3FF) << 52) | mantissa;  // exponent 1023 = 1.0
  double res;
  std::memcpy(&res, &bits, sizeof(bits));             // safely reinterpret
  return res - 1.0;  // in [0.0, 1.0)
}
